...
The 37 approved pathway recipes come from the standalone `585SwineMerchant/GCSD-Advanced-Culinary` repository's pathway recipe file. The recovered library was reconstructed from the surviving personal source photographs and reconciled against the 171-record Advanced Culinary Recipe Source Catalog. This migration copies culinary recipe content only; it does not connect to the old Cloudflare Worker or D1 database and does not import accounts, events, submissions, application state, or audit records. Of the 171 catalog records, 110 complete production recipes are added as review-only drafts, three recovered master-formula references remain audit-only, and 58 records still require complete source photographs. Every migrated recipe retains a visible teacher-verification warning for allergens, substitutions, holding limits, temperatures, and current food-safety requirements.
