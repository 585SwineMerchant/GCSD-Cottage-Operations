...
### Ingredient transcription correction (v0.12.1)

1. In the existing **GCSD Cottage Operations - Teacher** Apps Script project, replace `Code.gs` and `RecoveredRecipes.gs` with the repository versions. Do not change `Index.html`.
2. Save both files and run `initializeWorkbook()` once. It replaces ingredients only for recovered records that are still untouched Drafts with no approved version. Teacher-edited or approved recipes are skipped.
3. Review the execution result and audit row. Untouched recovered drafts should be updated; deliberately edited records must be reconciled manually against `data/recovered-recipes.json`.
4. Choose **Deploy → Manage deployments → Edit**, select **New version**, and deploy without changing the existing GCSD-domain access policy or `/exec` URL.
5. Open **Recipes**, spot-check R001, R008, R044, R093, and R138, and confirm all recovered records remain Drafts.
6. Do not update the public-feed project, recipe-import project, GitHub Pages configuration, or any Cloudflare project for this correction.
