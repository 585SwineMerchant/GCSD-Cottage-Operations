(() => {
  ...
  let studioImageFile=null,studioOcrPromise=null;
  ...
  async function readStudioScreenshot(){const file=studioImageFile||q("#studioScreenshot").files[0];if(!file){studioIntakeStatus("Choose or paste a screenshot first.",true);return}if(!/^image\//.test(file.type)||file.size>10*1024*1024){studioIntakeStatus("Use a PNG, JPEG, or WebP screenshot smaller than 10 MB.",true);return}const button=q("#readStudioScreenshot");button.disabled=true;let worker;try{studioIntakeStatus("Loading the on-device screenshot reader…");const Tesseract=await loadStudioOcr();worker=await Tesseract.createWorker("eng",1,{logger:message=>{if(message.status==="recognizing text")studioIntakeStatus(`Reading screenshot… ${Math.round(Number(message.progress||0)*100)}%`)}});const result=await worker.recognize(file),text=String(result?.data?.text||"").trim();if(!text)throw new Error("No readable recipe text was found. Try a tighter, clearer screenshot or paste the text.");q("#studioIntakeText").value=text;applyRecipeExtraction(globalThis.GCSDRecipeParser.parseRecipeText(text))}catch(error){studioIntakeStatus(error.message||String(error),true)}finally{if(worker)try{await worker.terminate()}catch(_){/* worker cleanup is best effort */}button.disabled=false}}
  ...
  q("#studioScreenshot").onchange=event=>{studioImageFile=event.target.files[0]||null;q("#studioDropZone").classList.toggle("ready",Boolean(studioImageFile));studioIntakeStatus(studioImageFile?`${studioImageFile.name} is ready to read.`:"Nothing has been read yet.")};
  q("#studioDropZone").addEventListener("paste",event=>{const file=[...(event.clipboardData?.files||[])].find(item=>/^image\//.test(item.type));if(!file)return;event.preventDefault();studioImageFile=file;q("#studioDropZone").classList.add("ready");studioIntakeStatus("Pasted screenshot is ready to read.")});
