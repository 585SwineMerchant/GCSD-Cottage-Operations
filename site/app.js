(() => {
  ...
  function loadJsonp(url, parameters={}, serviceLabel="publication feed") {
    return new Promise((resolve, reject) => {
      ...
      const timeout = window.setTimeout(() => finish(new Error(`The ${serviceLabel} did not respond.`)), 20000);
      window[callback] = value => finish(null, value);
      script.onerror = () => finish(new Error(`The ${serviceLabel} could not be loaded.`));
      const separator = url.includes("?") ? "&" : "?";
      const query = new URLSearchParams({ ...parameters, callback, _: String(Date.now()) });
  ...
  function applyRecipeExtraction(parsed){if(!parsed)return;const hasDraft=["studioName","studioIngredients","studioProcedure"].some(id=>q(`#${id}`).value.trim());if(hasDraft&&!window.confirm("Replace the current title, ingredients, and procedure with the extracted recipe?"))return;const values={name:parsed.name,category:parsed.category,standardYieldQuantity:parsed.standardYieldQuantity||"",standardYieldUnit:parsed.standardYieldUnit,ingredientsText:parsed.ingredientsText,procedureText:parsed.procedureText,equipmentText:parsed.equipmentText,allergens:parsed.allergens,sourceNotes:parsed.sourceNotes};Object.entries(values).forEach(([key,value])=>{if(value!==undefined&&value!==null&&String(value).trim()!=="")q(`#${studioFields[key]}`).value=value});const warnings=q("#studioWarnings"),messages=parsed.warnings||[];warnings.hidden=!messages.length;warnings.innerHTML=messages.length?`<strong>Check these fields:</strong><ul>${messages.map(message=>`<li>${esc(message)}</li>`).join("")}</ul>`:"";studioIntakeStatus(`Read ${Number(parsed.extractedLineCount||0)} lines. Review every field before exporting.`);saveStudioDraft();q("#studioName").scrollIntoView({behavior:"smooth",block:"center"})}
  async function importStudioUrl(){const url=q("#studioRecipeUrl").value.trim(),button=q("#importStudioUrl");if(!/^https:\/\//i.test(url)){studioIntakeStatus("Paste the complete HTTPS address of a recipe webpage.",true);return}if(!config.recipeImportUrl){studioIntakeStatus("Recipe URL import has not been connected yet. Use copied text until the import service is deployed.",true);return}button.disabled=true;button.textContent="Importing…";try{studioIntakeStatus("Reading the webpage's structured recipe data…");const response=await loadJsonp(config.recipeImportUrl,{url},"recipe import service");if(!response?.ok||!response.recipe)throw new Error(response?.error||"This webpage did not return a structured recipe.");const parsed=globalThis.GCSDRecipeParser.parseRecipeData(response.recipe);parsed.warnings=[...(response.warnings||[]),...(parsed.warnings||[])];applyRecipeExtraction(parsed)}catch(error){studioIntakeStatus(error.message||String(error),true)}finally{button.disabled=false;button.textContent="Import recipe URL"}}
