...
The Recipe Library begins with 37 approved pathway recipes migrated from the standalone `GCSD-Advanced-Culinary` source library, plus 110 recovered Advanced Culinary source transcriptions held as review-only drafts. The approved migration creates immutable version-one snapshots; recovered transcriptions create no approved version until a teacher deliberately reviews and approves them. Both seed operations are safe to rerun and copy no Cloudflare users, events, submissions, application state, or audit records. Imported recipes explicitly require teacher verification of allergens and food-safety controls before production.
...
The public Recipe Studio now treats a recipe webpage URL as the primary intake method. A separate, anonymous, read-only Recipe Import Apps Script fetches public HTTPS pages and returns only their standardized Recipe JSON-LD fields; it has no workbook or Drive access and stores no submissions. Exact webpage ingredient strings and instructions therefore avoid screenshot OCR errors such as `1 1/2` becoming `3.5`. Copied text and locally processed, cropped screenshot OCR remain fallbacks. Students review the structured draft, save it in their browser, then copy or download an identity-free export for protected teacher review. The public site retains no write path into district records.
...
- `apps-script/recipe-import/` — separate anonymous URL reader that extracts public Recipe JSON-LD and cannot access GCSD records
