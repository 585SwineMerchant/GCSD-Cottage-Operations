...
The Recipe Library begins with 37 approved pathway recipes migrated from the standalone `GCSD-Advanced-Culinary` source library. The migration copies only recipe content, creates immutable version-one snapshots, and is safe to rerun; it does not read or copy Cloudflare users, events, submissions, application state, or audit records. Imported recipes explicitly require teacher verification of allergens and food-safety controls before production.
