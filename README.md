...
`Google Form request → protected teacher application → private Event Order draft → costing, budget, and production planning → deliberate publication → read-only student site → Google operations documents`

The Teacher Command Center now includes a responsive operations dashboard, request queue, individual event workspaces, structured publication validation, separate operational and publication states, privacy-safe preview, immutable revision history, unpublish/republish, clone, archive/restore, document history, event audit history, an approved Recipe Library with event-level scaling, a full production planner, and private budget management.
