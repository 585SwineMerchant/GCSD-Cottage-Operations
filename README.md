...
`Google Form request → protected teacher application → private Event Order draft → costing, budget, inventory, and production planning → deliberate publication → read-only student site → Google operations documents`

The Teacher Command Center now includes a responsive operations dashboard, request queue, individual event workspaces, structured publication validation, separate operational and publication states, privacy-safe preview, immutable revision history, unpublish/republish, clone, archive/restore, document history, event audit history, an approved Recipe Library with event-level scaling, a full production planner, and private budget and inventory management.
...
Funding accounts track allocated, committed, spent, credited, and available funds across payment methods such as the Wegmans card and approved-vendor purchase orders. Event budgets show estimated food cost, cost per guest, purchase requirements, and variance. Inventory uses an append-only movement ledger for receipts, usage, waste, and adjustments; marking an event purchase `Received` creates one auditable inventory receipt.
