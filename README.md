...
The public Recipe Studio is also local-only. Students can search Google, paste copied recipe text, or select/paste a screenshot. A browser-side parser proposes the title, yield, ingredients, procedure, equipment, and allergen information and flags anything it could not detect. Screenshot OCR runs in the browser through a lazily loaded Tesseract.js worker; screenshots are not uploaded to the GCSD project. Students review the draft, save it locally, then copy or download an identity-free structured export. A teacher pastes that export into the protected Recipe Library, where it opens as an unsaved draft and must pass the existing teacher review, versioning, and approval process. The public site retains no write path into district records.
